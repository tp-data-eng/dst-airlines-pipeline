...
| Reporting Method & Trigger Script | Output Artifact | Analytical Purpose & Details                                                                                                           |
| :--- | :--- |:---------------------------------------------------------------------------------------------------------------------------------------|
| **`plot_registration_coverage()`**<br>`pipeline_daily.py` | `registration_coverage.png` | Dual donut/bar chart tracking the percentage of mapped vs. `UNKNOWN_REG` tail numbers.                                                 |
| **`plot_fleet_coverage_audit()`**<br>`pipeline_monthly.py` | `fleet_coverage_audit.png` | Two-panel visual audit tracking total enriched-to-unknown model ratios and top resolved commercial airframe types.        |
| **`plot_top_aircraft_models()`**<br>`pipeline_daily.py` | `top_aircraft_models.png` | Horizontal bar chart of frequent aircraft models, highlighting unmapped entities in a distinct warning color.             |
| **`plot_top_hub_airlines()`**<br>`utils/run_visuals.py` | `top_hub_airlines.png` | Side-by-side horizontal bar charts comparing top active airlines operating across target hub airports (EGLL, LTFM, LFPG). |
...
* **Trigger:** `pipeline_daily.py`
* **Artifact:** `docs/figures/flight_altitude_distribution.png`
